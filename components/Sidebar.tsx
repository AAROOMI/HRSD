...
}> = ({ label, view, currentView, setView, isHighlighted = false, children }) => (
    <button
        onClick={() => setView(view)}
        className={`w-full flex items-center justify-center p-4 rounded-lg my-2 transition-all duration-300 group relative ${
            isHighlighted ? 'highlight-tour-element' : ''
        } ${
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, tourState }) => {
    return (
        <nav className="flex-shrink-0 w-20 bg-black/30 backdrop-blur-lg border-r border-white/10 p-2 flex flex-col items-center print:hidden">
            ...
            <NavItem label="Template Library" view="templateLibrary" currentView={currentView} setView={setView}>
            ...
            <NavItem 
                ...
                setView={setView}
                isHighlighted={tourState.isActive && tourState.step === 8}
            >
            ...
            <NavItem 
                ...
                currentView={currentView} 
                setView={setView}
                isHighlighted={tourState.isActive && tourState.step === 7}
            >
                 <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" strokeWidth={2}>
